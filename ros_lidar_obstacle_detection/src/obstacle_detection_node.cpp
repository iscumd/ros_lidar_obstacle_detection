...
int main( int argc, char** argv )
{
    ...
    ros::Subscriber laserScanSubscriber = n.subscribe("/scan", 0, laserScanCallback);
    ros::spinOnce();
    ros::Publisher obstaclesPublisher = n.advertise<visualization_msgs::MarkerArray>("obstacle_detection/obstacles", 5);
    ros::Publisher filteredPointsPub = n.advertise<sensor_msgs::PointCloud>("filtered_scan", 0);
    ...
    n.param("filter_x_size", filter_x_size, 5.);
    ...
    while(n.ok()) {
        ...
        try {
            ...
            listener.waitForTransform("/odom", "/sick_laser_link", now, ros::Duration(2));
            listener.transformPointCloud("/map", now, cloud, "/sick_laser_link", transformed_cloud);
        }
        ...
        if (transformed_cloud.points.size() > 1) {
            ...
            filteredPoints = ISC::filtering::filter_by_boundary(
                    ISC::geometry::RectangleBoundary(filter_x_size, filter_y_size,
                                                     ISC::geometry::Point2D(filter_origin_x, filter_origin_y)), points);
            sensor_msgs::PointCloud filteredPointCloud;
            filteredPointCloud.header = transformed_cloud.header;
            filteredPointCloud.header.stamp = ros::Time::now();
            filteredPointCloud.header.frame_id = "/map";
